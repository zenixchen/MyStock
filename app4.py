...
# ==========================================
st.set_page_config(
    page_title="2026 量化戰情室 (Ultimate v9.3)",
    page_icon="🛡️",
    layout="wide",
...
st.title("🛡️ 量化戰情室 (Ultimate v9.3)")
st.caption("防彈版：修復 Series Ambiguous 錯誤 | 強制單值轉換 | 去重防護")
...
def get_safe_data(ticker):
    try:
        ...
        # ★ v9.3 關鍵：移除重複欄位 (Duplicate Columns Fix)
        # 有時候 yfinance 會回傳兩個 'Close'，導致後續計算變成 DataFrame 而非 Series
...
def analyze_ticker(config, ai_provider, api_key_groq, api_key_gemini, gemini_model_name, enable_debate):
    ...
    if lp is None: lp = float(df['Close'].iloc[-1])
...
st.divider()
st.success("✅ 分析完成 (v9.3 Ultimate - Series Fix + Force Float)")
