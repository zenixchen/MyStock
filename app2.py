...
st.caption("15檔核心持股 (原始策略) + LLM 深度推演 (標題+摘要) | KeyError 修復版")
...
def analyze_ticker(config, groq_client=None):
    ...
    if config['mode'] == "SUPERTREND":
        ...
        strategy_desc = f"布林+RSI (破下軌且RSI<{config['entry_rsi']} 買)"
