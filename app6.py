...
# ------------------------------------------
    ...
    test_ratio = st.sidebar.slider("回測長度 (Test Size)", 0.05, 0.5, 0.2, 0.05)
    
    # 計算 split 的邏輯也建議放在外面，或者在裡面重新計算
    # 這裡我們只放滑桿，變數 test_ratio 會自動傳進下面的按鈕裡
    # ==========================================
    ...
    if st.button(f"🚀 啟動 {target} AI 訓練"):
        with st.spinner(f"正在召喚 {model_mode.split()[1]} AI 模型..."):
            try:
                ...
                st.sidebar.divider()
                st.sidebar.header("🔧 回測時光機")
                # 預設 0.2 (20%)，拉到 0.1 代表只看最近 10% 的日子 (紅線變短)
                test_ratio = st.sidebar.slider("回測長度 (Test Size)", 0.05, 0.5, 0.2, 0.05)
